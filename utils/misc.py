...
def log_model_profile(model, npoints, logger=None):

    base_model = unwrap_model(model)

    trainable_params = sum(v.numel() for v in base_model.parameters() if v.requires_grad)

    print_log('>> {:25s}\t{:.2f}'.format('#TP(M):', trainable_params / (1.0 * 10 ** 6)), logger)
    ...
    try:
        ...
        print_log('>> {:25s}\tN/A'.format('GFLOPS:'), logger)

        return {'tp_m': trainable_params / (1.0 * 10 ** 6), 'gflops': None}
    ...
    if hasattr(base_model, 'get_profile_inputs'):

        profile_inputs = base_model.get_profile_inputs(npoints, device)

        if not isinstance(profile_inputs, tuple):

            profile_inputs = (profile_inputs,)

    else:

        profile_inputs = (torch.randn(1, npoints, 3, device=device),)
    ...
    try:
        ...
        with torch.no_grad():
            ...
            macs, _ = profile(base_model, inputs=profile_inputs, verbose=False)
        ...
        try:
            ...
            with torch.no_grad():
                ...
                flop_counter = FlopCountAnalysis(base_model, profile_inputs)
                ...
                gflops = flop_counter.total() / (10 ** 9)
            ...
            try:
                ...
                with profile(activities=activities, with_flops=True) as prof:
                    ...
                    with torch.no_grad():
                        ...
                        base_model(*profile_inputs)

                gflops = sum(evt.flops for evt in prof.key_averages()) / (10 ** 9)
    ...
    if gflops is None:
        ...
        print_log('>> {:25s}\t{:.2f}'.format('GFLOPS:', gflops), logger)



    return {'tp_m': trainable_params / (1.0 * 10 ** 6), 'gflops': gflops}
